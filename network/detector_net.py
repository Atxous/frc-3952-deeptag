...
class SSDRegressor(nn.Module):
    ...
    def forward(self, x):
        ...
        #     locations.append(l)
        #     vals.append(v)
        for idx, (stride, reg) in enumerate(zip(self.stride_features, self.regression_headers)):
          if idx > 0:
            x1 = stride(x1)
          c, l, v = reg(x1)
          confidences.append(c)
...
class SSDRegressionHeaders(nn.Module):
    ...
    def reshape_features(self, features, num_box_versions:int=1):
        if features.dim()==4:  
    ...
    def forward(self, x):
        ...
        locations = self.reshape_features(locations, num_box_versions)
        confidences = self.reshape_features(confidences, num_box_versions)
        vals = self.reshape_features(vals, num_box_versions)
